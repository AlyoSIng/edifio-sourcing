...
/**
 * RoadmapPdfForm — formulaire de configuration / modification de l'URL
 * de la roadmap produit PDF (Google Drive, Dropbox, etc.)
 *
 * Client Component utilisé dans la page Roadmap superadmin.
 * Appelle `saveRoadmapUrlAction` via useTransition pour rester non bloquant.
 *
 * Props :
...
import { saveRoadmapUrlAction } from "./actions";

interface RoadmapPdfFormProps {
...
/**
 * Formulaire de configuration de l'URL du PDF roadmap produit.
 * Affiche un champ URL + bouton de sauvegarde.
 * Erreurs affichées inline sous le champ.
 */
export function RoadmapPdfForm({ initialUrl = "", onSuccess }: RoadmapPdfFormProps) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await saveRoadmapUrlAction(trimUrl);
      if (!result.ok) {
  ...
  return (
    <div className="rounded-md border border-line bg-white p-5 shadow-sm">
      <h3 className="mb-1 text-sm font-semibold text-ink">
        {initialUrl ? "Modifier l'URL de la roadmap" : "Configurer l'URL de la roadmap"}
      </h3>
      <p className="mb-4 text-xs text-muted">
        Collez le lien de partage de votre PDF roadmap (Google Drive « partager → lien », Dropbox,
        etc.). Le PDF sera affiché directement sur cette page.
      </p>

      <form onSubmit={handleSubmit} className="space-y-3">
        <div>
          <label htmlFor="roadmap-pdf-url" className="mb-1 block text-xs font-medium text-ink-2">
            URL du PDF roadmap <span className="text-error">*</span>
          </label>
          <input
            id="roadmap-pdf-url"
            type="url"
