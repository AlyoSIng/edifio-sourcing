...
/**
 * RoadmapPdfViewer — affichage du PDF via <object> + toggle formulaire de modification
 *
...
interface RoadmapPdfViewerProps {
  /** URL configurée dans `app_content`. */
  url: string;
}

/**
 * Affiche le PDF de la roadmap ou le formulaire de modification.
 * Le bouton "Modifier l'URL" bascule entre les deux vues.
 */
export function RoadmapPdfViewer({ url }: RoadmapPdfViewerProps) {
  const [isEditing, setIsEditing] = useState(false);

  if (isEditing) {
    return <RoadmapPdfForm initialUrl={url} onSuccess={() => setIsEditing(false)} />;
  }
