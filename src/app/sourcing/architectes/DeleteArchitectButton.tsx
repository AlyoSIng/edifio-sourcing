...
/**
 * Bouton de suppression hard d'un architecte — admin uniquement.
 *
...
import { deleteArchitectAction } from "./actions";

interface DeleteArchitectButtonProps {
  /** UUID de l'architecte à supprimer. */
  architectId: string;
  /** Nom du cabinet — affiché dans le texte de confirmation. */
  cabinet: string;
}

export function DeleteArchitectButton({ architectId, cabinet }: DeleteArchitectButtonProps) {
  const [state, setState] = useState<"idle" | "confirm" | "pending">("idle");
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  // Synchronisation état local avec la transition React
  ...
  function handleClickConfirm() {
    ...
    startTransition(async () => {
      const result = await deleteArchitectAction(architectId);

      if (result.ok) {
        // Rafraîchissement complet de la liste (Server Component)
      ...
      switch (result.error) {
        case "has_active_solicitations":
          setErrorMsg("Impossible — sollicitations en cours.");
          setState("idle");
          break;
        case "not_found":
          // Déjà supprimé par un autre admin — on recharge silencieusement
          window.location.reload();
          break;
        default:
