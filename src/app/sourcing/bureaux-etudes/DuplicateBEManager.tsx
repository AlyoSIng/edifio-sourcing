...
import { deleteBEDuplicateAction } from "./duplicate-be-actions";

interface Props {
  duplicateGroups: DuplicateBEGroup[];
}

/**
 * Bandeau de gestion des doublons bureaux d'études — Client Component.
 *
 ...
 *   - Bouton "Supprimer" sur chaque entrée (sauf la dernière du groupe)
 *
 * @param duplicateGroups - Groupes de doublons détectés côté serveur.
 */
export function DuplicateBEManager({ duplicateGroups: initialGroups }: Props) {
  const [groups, setGroups] = useState<DuplicateBEGroup[]>(initialGroups);
  ...
  function handleDelete(id: string) {
    ...
    startTransition(async () => {
      const result = await deleteBEDuplicateAction(id);
      ...
      setGroups((prev) => {
        const updated = prev
          ...
          // Supprimer les groupes qui n'ont plus qu'une entrée
...
interface PanelProps {
  group: DuplicateBEGroup;
  isPending: boolean;
  errors: Record<string, string>;
  onDelete: (id: string) => void;
}

function DuplicateBEGroupPanel({ group, isPending, errors, onDelete }: PanelProps) {
  return (
    <div className="rounded-md border border-line bg-white">
      ...
      <ul className="divide-y divide-line">
        {group.entries.map((entry, idx) => {
          // La dernière entrée ne peut pas être supprimée
...
function formatError(error: string): string {
  switch (error) {
    case "has_active_solicitations":
      return "Impossible de supprimer : ce bureau d'études a des sollicitations ou propositions de matching en cours.";
    case "not_found":
