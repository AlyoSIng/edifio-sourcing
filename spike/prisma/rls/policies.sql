-- ============================================================================
-- rls/policies.sql — RLS policies (Spike Prisma Phase 2b)
-- ----------------------------------------------------------------------------
-- VOLONTAIREMENT IDENTIQUE à spike/drizzle/rls/policies.sql.
-- Justification : Prisma ne génère PAS les RLS (limitation connue ORM
-- application-layer-first). Le SQL natif est donc inévitable côté Prisma —
-- et puisque ce SQL est exactement le même que celui qu'on écrit côté
-- Drizzle (Drizzle 0.39 supporterait `pgPolicy` côté schema TS mais on a
-- gardé le SQL manuscrit en Phase 2a pour permettre cette comparaison
-- équitable), le critère « compat Supabase + RLS » 15 % Gate 5 ne discrimine
-- pas les deux ORMs sur la définition (ligne pour ligne identique).
--
-- Là où la comparaison se fait, c'est sur la chaîne d'application :
--   - Drizzle : on PEUT exprimer les RLS en TS via pgPolicy (DX bonus
--     éventuel Phase 4). On NE LE FAIT PAS en spike pour rester comparable.
--   - Prisma : on NE PEUT PAS exprimer les RLS en schema.prisma (au mieux
--     un trigger ou une fonction définie via prisma migrate via fichier SQL
--     externe). Le SQL manuscrit est obligatoire. Cf. rapport Phase 4.
--
-- À appliquer APRÈS la migration Prisma (`prisma/migrations/0000_init.sql`) :
--   psql $DATABASE_URL -f prisma/migrations/0000_init.sql
...
-- le schema Prisma 2b — on retombe sur text pour ne pas créer une dépendance
-- inutile).
...
-- en production via trigger ou check expression — hors Phase 2b).
