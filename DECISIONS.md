...
- **2026-05-18 · DEV Alex · Fix CI `ci-db-rls` — installer pgtap dans le container Postgres du service GHA.** [FIX CI]
  *Le fix PGDG précédent installait `postgresql-15-pgtap` sur le runner host (utile pour `pg_prove` + le module Perl `TAP::Parser::SourceHandler::pgTAP`), mais insuffisant : le service `postgres:15` tourne dans un container Docker isolé qui n'a pas accès aux paquets apt de l'host. `CREATE EXTENSION pgtap` plantait dans l'étape « Prepare Supabase auth schema stub » sur `Could not open extension control file pgtap.control: No such file or directory`. Ajout d'une étape qui résout le container via `docker ps --filter ancestor=postgres:15` puis lance `apt-get install postgresql-15-pgtap` à l'intérieur via `docker exec`. L'image officielle `postgres:15` est basée Debian bookworm et embarque déjà le dépôt PGDG, donc le paquet est trouvable sans configuration supplémentaire.*

- **2026-05-19 · DEV Alex · Fix seed — stub `auth.users` (peupler en miroir avant `public.users`).** [FIX SEED]
  *4e dérive CI consécutive sur la 1re PR module sourcing engine : `insert or update on table "users" violates foreign key constraint "users_id_auth_users_fk"` à l'étape « Seed dev/CI dataset ». La FK `users.id → auth.users(id) ON DELETE CASCADE` posée par `0003_fk_supabase.sql` bloque l'insert si `auth.users` est vide. En prod, Supabase Auth peuple `auth.users` automatiquement lors de l'invitation user. En dev/CI, le stub `auth.users (id uuid PK, email text)` existe (workflow + script) mais reste vide. Fix : avant le `db.insert(users)`, insérer les mêmes UUIDs dans `auth.users` via boucle `INSERT ... ON CONFLICT (id) DO NOTHING`. Idempotent. Pas de guard prod (le seed est `JAMAIS sur prod` par convention CLAUDE.md, et le contenu seedé inclut une `Seed Test Org B` clairement dev/CI-only). **4e dérive révélée à la 1re exécution Postgres réelle** (pgtap host repo, pgtap container install, NOW() index, auth.jwt() stub, auth.users seed FK) — chacune aurait été attrapée par un dry-run local 30 s. Cf. memory `feedback-postgres-dry-run-local`.*

- **2026-05-19 · DEV Alex · Fix CI `ci-db-rls` — stub `auth.jwt()` au même titre que `auth.users`.** [FIX CI]
  *3e bug Postgres CI consécutif sur la 1re PR module sourcing engine : `function auth.jwt() does not exist` au moment de l'application de `0002_rls.sql`. La policy `tenant_isolation` sur `notifications` y fait référence (cf. `0002_rls.sql:184` : `user_id = (auth.jwt() ->> 'sub')::uuid`). En prod Supabase, `auth.jwt()` est posée par Supabase Auth ; en CI vanilla Postgres, elle est absente. Stub ajouté dans l'étape « Prepare Supabase auth schema stub » du workflow `db-rls.yml` ET dans `scripts/db-dry-run.ps1` (cohérence) : `CREATE OR REPLACE FUNCTION auth.jwt() RETURNS jsonb AS $$ SELECT NULLIF(current_setting('request.jwt.claims', true), '')::jsonb $$ LANGUAGE sql STABLE;`. Comportement en CI : retourne NULL en l'absence de session simulée (acceptable, les pgTAP simuleront via `SET LOCAL request.jwt.claims = '...'` au cas par cas). 3e dérive CI consécutive révélée à la 1re exécution Postgres réelle (pgtap host, pgtap container, NOW() index, auth.jwt()) — confirme la pertinence absolue de la nouvelle memory `feedback-postgres-dry-run-local` : aucune de ces 3 dérives n'aurait passé un dry-run local 30 s.*

- **2026-05-18 · DEV Alex · Fix DB — retirer `NOW()` du prédicat `idx_tenders_deadline` (SQLSTATE 42P17).** [FIX DB + POST-MORTEM SPEC]
  *La 1re exécution Postgres réelle de `0001_schema_v1.sql` en CI a révélé un bug latent : Postgres refuse `CREATE INDEX … WHERE deadline > now()` car `now()` est `STABLE`, pas `IMMUTABLE` (les prédicats d'index doivent être déterministes). Erreur `PostgresError: functions in index predicate must be marked IMMUTABLE` (code 42P17, file `indexcmds.c:1748`, routine `CheckPredicate`). **Bug latent dans `specs/schema_v1.sql:206`** — la spec CTO Sophie posait le même DDL fautif, Drizzle l'a fidèlement repris. Audit complet des 10 prédicats d'index `.where()` du schema Drizzle : seul `idx_tenders_deadline` était fautif ; tous les autres utilisent des prédicats IMMUTABLE (`IS NULL`, `IS NOT NULL`, `= constante`, référence directe à une colonne boolean). Option (a) retenue : retirer le `.where()` et garder un index full sur `deadline`. Justification : volume cible ~100-400 K rows sur 12 mois → index full ~10-30 MB acceptable ; toutes les queries applicatives filtrent `deadline > now()` côté WHERE clause → range scan sur l'index full, même perf qu'un partiel. Option (c) avec colonne `is_active` maintenue par trigger/cron rejetée pour complexité opérationnelle disproportionnée (YAGNI). Édit en place dans `0001_schema_v1.sql` + 3 snapshots `meta/000{1,2,3}_snapshot.json` + `src/db/schema/tenders.ts` (la PR n'est pas encore mergée, pas besoin de migration 0004 DROP+CREATE parasite). `pnpm drizzle-kit check` = zéro drift. **Action CTO Sophie** : amender `specs/schema_v1.sql:206` côté source de vérité pour aligner sur le code livré.*

- **2026-05-19 · DEV Alex · Fix pgTAP RLS — rôle non-superuser pour bypass RLS superuser PG 15 + collision seed BOAMP.** [FIX CI + POST-MORTEM TESTS]
  *PR #14 OPEN — 2 jobs `ci-db-rls` rouges, tous les autres CI verts (lint, typecheck, test, build, e2e, middleware, Vercel preview). Deux bugs distincts cumulés sur la même pipeline.
  **Bug A (collision seed)** : `tests/rls/02_tenant_isolation.sql:71-72` insérait `platforms(code='boamp', ...)`, déjà inséré par `pnpm db:seed` qui tourne désormais avant pgTAP (cf. workflow étendu 2026-05-18). Violation `UNIQUE platforms_code_unique` → exit 3 pgTAP → « Bad plan: 9 planned 0 ran ». Fix : `code='boamp_test'` + `name='tender_score_full_test'` (par précaution, même symptôme potentiel). Le test 02 a désormais ses propres fixtures isolées du seed dev/CI.
  **Bug B (superuser bypass RLS)** : `pg_prove` connecte en `postgres` (superuser). En PG 15, le superuser bypass RLS **inconditionnellement** — `FORCE ROW LEVEL SECURITY` n'affecte QUE le table-owner, pas les superusers (by-design). Conséquence : (a) sur `03_insert_by_member.sql`, test 3 (viewer INSERT) — RLS ignorée donc INSERT réussit → `throws_ok 42501` reçoit `no exception` → FAIL ; test 4 (admin OrgA INSERT pour OrgB) — RLS ignorée donc la FK fire en premier → reçoit `23503` au lieu de `42501` → FAIL ; (b) sur `02_tenant_isolation.sql`, une fois Bug A corrigé, les 6 assertions cross-tenant verraient toutes les lignes des deux orgs. Fix : nouvelle étape workflow `Create non-superuser role for RLS assertions` qui crée un rôle applicatif `test_authenticated` (NOINHERIT NOLOGIN, sans BYPASSRLS) entre `Run migrations` et `Seed dev/CI dataset` ; chaque fichier pgTAP ajoute `SET LOCAL ROLE test_authenticated` avant la phase d'assertion (le setup reste en `postgres` pour insérer les fixtures sans contraintes RLS — bypass volontaire et borné au BEGIN ... ROLLBACK).
  3 fichiers modifiés : `.github/workflows/db-rls.yml` (nouvelle étape rôle) + `tests/rls/02_tenant_isolation.sql` (renaming + `SET LOCAL ROLE`) + `tests/rls/03_insert_by_member.sql` (`SET LOCAL ROLE`). **Dry-run local** : à exécuter par Yann via `pnpm db:dry-run` avant le commit (`scripts/db-dry-run.ps1` couvre migrations + seed + pgTAP si pg_prove dispo). **5e dérive Postgres CI consécutive** (pgtap host, pgtap container, NOW() index, auth.jwt() stub, auth.users seed, superuser bypass + collision seed) — chacune aurait été attrapée par un dry-run local 30 s. Cf. memory `feedback-postgres-dry-run-local` (de plus en plus pertinente).*

- **2026-05-19 · DEV Alex · Fix pgTAP RLS itération 2 — enum `platform_code` + policy `AS RESTRICTIVE`.** [FIX CI + POST-MORTEM TESTS]
  *PR #14 OPEN — commit précédent `e7c7d5b` (itération 1) a partiellement débloqué `ci-db-rls` : test 04 audit immutable OK, test 4 cross-tenant admin OrgA → OrgB OK (le `SET LOCAL ROLE test_authenticated` a réglé le bypass superuser PG 15). 2 nouveaux bugs cumulés à l'exécution.
  **Bug C (collision enum)** : `tests/rls/02_tenant_isolation.sql:74` insérait `code='boamp_test'`, mais `platforms.code` est un enum `platform_code` (cf. `0001_schema_v1.sql:9`, valeurs `boamp|place|francmarches|mp_info`), donc valeur custom rejetée par Postgres avant même la contrainte UNIQUE. Le commit itération 1 avait corrigé Bug A en renommant `'boamp'` → `'boamp_test'` pour éviter la collision UNIQUE, sans réaliser que la colonne était enum-typée. Toutes les 4 valeurs enum sont déjà occupées par le seed dev/CI. Fix : réutiliser la ligne `'boamp'` du seed via `ON CONFLICT (code) DO NOTHING` + `SELECT id FROM platforms WHERE code = 'boamp'` en subquery pour `platform_id` dans les INSERT tenders. Le UUID hard-codé `cccc0000-0000-0000-0000-000000000001` reste référencé dans les VALUES mais n'a pas d'effet en cas de pré-existence (le seed pose son propre UUID).
  **Bug D (PERMISSIVE OR'd)** : `tests/rls/03_insert_by_member.sql` test 3 (viewer INSERT dans sa propre org) recevait `no exception` au lieu de `42501`. Cause : Postgres applique les policies PERMISSIVE en **OR** (au moins une autorise) et les RESTRICTIVE en **AND** (toutes doivent autoriser). Sur `architects`, deux policies PERMISSIVE par défaut — `tenant_isolation` (FOR ALL, org match) et `insert_by_member` (FOR INSERT, org match AND role ∈ admin/user). Pour un viewer OrgA → OrgA : `tenant_isolation.WITH CHECK` = TRUE (org match), `insert_by_member.WITH CHECK` = FALSE (role check), OR final = TRUE → INSERT autorisé → test échoue. Le test 4 cross-tenant passait déjà car les deux policies retournent FALSE (org mismatch). Fix : `CREATE POLICY "insert_by_member" ON "architects" AS RESTRICTIVE FOR INSERT WITH CHECK (...)`. Validation 4 scénarios test 03 : admin OK (TRUE AND TRUE), user OK (TRUE AND TRUE), viewer 42501 (TRUE AND FALSE), admin cross-tenant 42501 (FALSE AND FALSE).
  **Dérive spec↔runtime** : `specs/schema_v1.sql:551` portait le même bug (policy déclarée sans `AS RESTRICTIVE`). Édité en bundle pour aligner la source de vérité — 3e dérive spec/runtime de la 1re PR module sourcing engine (après `idx_tenders_deadline` NOW() bug 2026-05-18 et FK `users.id → auth.users(id)` 2026-05-19). Pas d'action CTO Sophie séparée requise — alignement committé en même temps que le fix code.
  5 fichiers modifiés : `tests/rls/02_tenant_isolation.sql` + `src/db/migrations/0002_rls.sql` + `specs/schema_v1.sql` + `DECISIONS.md` (cette entrée) + `notes-de-suivi/CC_260519_1730_FIX_PGTAP_RLS_IT2.md`. Pas de `drizzle-kit generate` : Drizzle ne modélise pas les policies RLS (conforme ADR-013, RLS via SQL natif hors ORM), aucun drift sur les snapshots meta. **6e dérive Postgres CI consécutive** sur la même PR (pgtap host, pgtap container, NOW() index, auth.jwt() stub, auth.users seed, superuser bypass + collision seed, enum + PERMISSIVE OR'd). Recommandation amendement memory `feedback-postgres-dry-run-local` : **RLS policy testing nécessite (1) `pg_prove` ou équivalent, (2) rôle non-superuser sans BYPASSRLS, (3) analyse explicite des combinaisons PERMISSIVE/RESTRICTIVE × USING/WITH CHECK × FOR ALL/INSERT/UPDATE/DELETE avant tout `CREATE POLICY` non-trivial**. Le test mental « est-ce que cette policy par elle-même autorise X ? » est insuffisant — il faut tester « est-ce que l'UNION des policies de la table autorise X ? ».*
