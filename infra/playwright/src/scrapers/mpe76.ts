 ...
 *  - Page de liste   : https://www.mpe76.fr/appels-offres/
 *  - Résultats       : `.liste-ao li`, `table tbody tr`, `[class*='consultation']`
 *  - Référence AO    : extrait du slug d'URL /consultation/<REF> ou /ao/<REF>
 *  - Pagination      : lien ou bouton "Suivant" / paramètre GET `?page=N`
 *
...
const BASE_URL = "https://www.mpe76.fr";
...
async function extractTenderDetail(
  ...
  try {
    ...
    console.warn("[mpe76] navigation fiche échouée:", href, err);
    return detail;
  }
...
async function extractListItems(page: Page): Promise<ListItem[]> {
  ...
  try {
    const rows = await page.$$(
      ".liste-ao li, table tbody tr, [class*='consultation'] li, [class*='consultation'] tr",
    );

    for (const row of rows) {
      try {
        // Lien vers la fiche détail
        const linkEl = await row.$("a[href*='/consultation/'], a[href*='/ao/']");
        if (!linkEl) continue;
        ...
        console.warn("[mpe76] extraction ligne échouée:", err);
      }
    }
  } catch (err) {
    console.warn("[mpe76] extraction liste échouée:", err);
  }
...
/**
 * Scrappe les appels d'offres récents sur MPE 76.
 *
 ...
 */
export async function scrapeMpe76(
  req: ScrapeRequest,
  browser: Browser,
  ...
  try {
    ...
    console.info(`[mpe76] navigation vers ${startUrl}`);
    try {
      ...
      console.error("[mpe76] échec navigation page initiale:", err);
      return [];
    }
    ...
    for (let pageIndex = 0; pageIndex < MAX_PAGES; pageIndex++) {
      if (Date.now() >= deadline) {
        console.warn("[mpe76] timeout global atteint à la page", pageIndex);
        break;
      }

      const items = await extractListItems(page);
      console.info(`[mpe76] page ${pageIndex + 1} — ${items.length} éléments`);
      ...
      for (const item of items) {
        ...
        try {
          ...
          console.warn("[mpe76] extraction AO échouée:", item.href, err);
    ...
    if (context) {
      try {
        ...
        console.warn("[mpe76] erreur fermeture context:", err);
      }
    }
  }

  console.info(`[mpe76] scraping terminé — ${tenders.length} AOs collectés`);
  return tenders;
}
