/**
 * Scraper Marchés Publics Normandie — marchespublics.normandie.fr
 *
 ...
 *  - Portail AWS + Drupal
 *  - Page de liste   : https://marchespublics.normandie.fr/
 *  - Résultats       : `.views-row`, `[class*='consultation']`,
 *                      `table.consultation tbody tr`
 *  - Référence AO    : extrait du slug d'URL /consultation/<REF> ou /avis/<REF>
...
const BASE_URL = "https://marchespublics.normandie.fr";
 ...
 * Ex: "/consultation/REF-123" → "REF-123"
...
async function extractTenderDetail(
  ...
  try {
    ...
    console.warn("[marchespublicsnormandie] navigation fiche échouée:", href, err);
    return detail;
  }
 ...
 * Portail Drupal — sélecteurs Drupal Views (.views-row) en priorité.
 */
async function extractListItems(page: Page): Promise<ListItem[]> {
  ...
  try {
    const rows = await page.$$(
      ".views-row, [class*='consultation'] li, [class*='consultation'] tr, table.consultation tbody tr",
    );

    for (const row of rows) {
      try {
        // Lien vers la fiche détail
        const linkEl = await row.$("a[href*='/consultation/'], a[href*='/avis/']");
        if (!linkEl) continue;
        ...
        const titleText = await row
          .$eval(
            "h2, h3, [class*='titre'], [class*='title'], [class*='objet'], .views-field-title",
            (el) => el.textContent?.trim() ?? "",
          )
          .catch(() => undefined);
        ...
        const buyerText = await row
          .$eval(
            "[class*='acheteur'], [class*='organisme'], .views-field-field-acheteur",
            (el) => el.textContent?.trim() ?? "",
          )
          .catch(() => undefined);
        ...
        const deadlineText = await row
          .$eval(
            "[class*='date'], [class*='echeance'], [class*='limite'], .views-field-field-date",
            (el) => el.textContent?.trim() ?? "",
          )
          .catch(() => undefined);
        ...
        console.warn("[marchespublicsnormandie] extraction ligne échouée:", err);
      }
    }
  } catch (err) {
    console.warn("[marchespublicsnormandie] extraction liste échouée:", err);
  }
...
/**
 * Scrappe les appels d'offres récents sur le portail Marchés Publics Normandie.
 *
 ...
 */
export async function scrapeMarChesPublicsNormandie(
  req: ScrapeRequest,
  browser: Browser,
  ...
  try {
    ...
    console.info(`[marchespublicsnormandie] navigation vers ${startUrl}`);
    try {
      ...
      console.error("[marchespublicsnormandie] échec navigation page initiale:", err);
      return [];
    }
    ...
    for (let pageIndex = 0; pageIndex < MAX_PAGES; pageIndex++) {
      if (Date.now() >= deadline) {
        console.warn("[marchespublicsnormandie] timeout global atteint à la page", pageIndex);
        break;
      }

      const items = await extractListItems(page);
      console.info(`[marchespublicsnormandie] page ${pageIndex + 1} — ${items.length} éléments`);
      ...
      for (const item of items) {
        ...
        try {
          ...
          console.warn("[marchespublicsnormandie] extraction AO échouée:", item.href, err);
    ...
    if (context) {
      try {
        ...
        console.warn("[marchespublicsnormandie] erreur fermeture context:", err);
      }
    }
  }

  console.info(`[marchespublicsnormandie] scraping terminé — ${tenders.length} AOs collectés`);
  return tenders;
}
