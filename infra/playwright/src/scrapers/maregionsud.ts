/**
 * Scraper Région Sud — marches-publics.maregionsud.fr
 *
 * Scraping non-authentifié du portail marchés publics Région Provence-Alpes-Côte d'Azur.
 *
 * Hypothèses sur la structure du site (portail de type Xdemat / AWS Marketplace) :
 *  - Page de liste    : https://marches-publics.maregionsud.fr/index.php
 ...
 *  - Pagination       : lien ou bouton "Suivant" / numéros de page ou ?page=N
 *
...
const BASE_URL = "https://marches-publics.maregionsud.fr";
...
async function extractTenderDetail(
  ...
  try {
    ...
    console.warn("[maregionsud] navigation fiche échouée:", href, err);
    return detail;
  }
  ...
  try {
    ...
    for (const row of rows) {
      try {
        ...
        console.warn("[maregionsud] extraction ligne échouée:", err);
      }
    }
  } catch (err) {
    console.warn("[maregionsud] extraction liste échouée:", err);
  }
...
async function goToNextPage(page: Page): Promise<boolean> {
  try {
    const nextLink = await page.$(
      "a[rel='next'], a[aria-label*='suivant' i], a[class*='next'], a:has-text('Suivant'), a:has-text('suivant'), [class*='pagination'] a:last-child, a[href*='page='][href*='numPage=']",
    );
...
/**
 * Scrappe les appels d'offres récents sur le portail Région Sud.
 *
 ...
 */
export async function scrapeMarEgionSud(
  req: ScrapeRequest,
  browser: Browser,
  ...
  try {
    ...
    console.info(`[maregionsud] navigation vers ${startUrl}`);
    try {
      ...
      console.error("[maregionsud] échec navigation page initiale:", err);
      return [];
    }
    ...
    for (let pageIndex = 0; pageIndex < MAX_PAGES; pageIndex++) {
      if (Date.now() >= deadline) {
        console.warn("[maregionsud] timeout global atteint à la page", pageIndex);
        break;
      }

      const items = await extractListItems(page);
      console.info(`[maregionsud] page ${pageIndex + 1} — ${items.length} éléments`);
      ...
      for (const item of items) {
        ...
        try {
          ...
          console.warn("[maregionsud] extraction AO échouée:", item.href, err);
    ...
    if (context) {
      try {
        ...
        console.warn("[maregionsud] erreur fermeture context:", err);
      }
    }
  }

  console.info(`[maregionsud] scraping terminé — ${tenders.length} AOs collectés`);
  return tenders;
}
